...
function twc_init()
{
    ...
    else
    {
        if(is_admin())
        {
	    if($_GET['page'] == 'twitconnect/twitconnect.php')
	    {
...
function twc_oAuth_Confirm()
{
    ...
    $xml = $to->OAuthRequest('http://api.twitter.com/1/account/verify_credentials.xml', array(), 'GET');
    ...
    if(!empty($shortlink))
    {
        ...
        if(strlen($tweet) <= 140)
        {
            if($twc_local == 'Y')
            {
                ...
                $to = new TwitterOAuth($twc_consumer_key, $twc_consumer_secret, $_SESSION['oauth_access_token'], $_SESSION['oauth_access_token_secret']);
                /* Run request on twitter API as user. */
                $content = $to->OAuthRequest('http://api.twitter.com/1/statuses/update.xml', array('status' => $tweet), 'POST');
            }
            else
            {
