...
function twc_get_avatar($avatar, $id_or_email='',$size='32') {
  ...
  if (get_usermeta($id_or_email, 'twcid')) {
    ...
    //Default to web version of SPIURL
    if(empty($twc_profile_images))
    {
        ...
        <tr>
          ...
          <td>
            <input type='text' name='twc_profile_images' value='<?php echo $twc_profile_images ?>' size="75" />
                 <br/><small>
                  Enter the link to the service that provides your Twitter profile images.  %%username%% will be replaced with the user's Twitter name.<br/>
                  If you are running the <a href="http://wordpress.org/extend/plugins/spiurl/" target="_blank">SPIURL</a> plugin, use this example:<br/> http://bloghome/?spiurl_user=%%username%%
                </small>
          </td>
        </tr>
